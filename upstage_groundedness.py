...
**⚠️ 중요 주의사항:**
- **CSV에서 확인된 회사 정보 (대표자명, 담당자명, 경쟁사, 사용PG 등)는 검증된 데이터이므로 환각이 아닙니다**
- **참조 문서 상단의 "CSV에서 확인된 회사 정보" 섹션에 있는 내용은 모두 사실이므로 유지하세요**
- **대표자명이나 담당자명이 CSV 데이터와 일치하면 환각이 아닙니다**
...
5. **CSV 데이터(대표자명, 담당자명, 사용PG 등)는 검증된 정보이므로 유지**

**수정 규칙:**
- ❌ "최근 시리즈 A 투자 유치" → 참조 문서에 없으면 삭제
- ❌ "급성장하고 있는" → 참조 문서에 없으면 삭제
- ✅ CSV에 명시된 대표자명/담당자명 → 검증된 데이터이므로 유지
- ✅ CSV에 명시된 사용PG 정보 → 검증된 데이터이므로 유지
