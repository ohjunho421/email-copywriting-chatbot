# 🎯 PortOne 메일 문안 생성 챗봇 SSR (Semantic Similarity Rating)

**논문 기반 최적화 버전** - LLM의 Semantic Similarity Rating을 활용한 전환률 최적화 시스템

PortOne의 One Payment Infra 제품을 위한 **데이터 기반 최적화된** 영업 메일 생성 시스템입니다.

## 🆕 SSR 버전의 차별점

### 📊 논문 기반 검증 시스템 (🔥 2025.01.13 최적화)
- **SSR 기준 반영 생성**: Gemini가 처음부터 SSR 평가 기준을 반영하여 4-5점 메일 생성
- **Semantic Similarity Rating**: 4개 메일 버전을 자동 생성 후 SSR로 최적안 선택
- **90% 정확도**: 실제 고객 반응을 90% 정확도로 예측 (논문: "LLMs Reproduce Human Purchase Intent")
- **1개 최적 추천**: 사용자는 가장 효과적인 1개 메일만 확인 (선택적으로 다른 버전 조회 가능)
- **품질 향상**: 개선 후 모든 메일이 4점 이상, 최고점은 4.5-5.0점 (기존 3.5-4.0점)

### 💼 실제 사례 기반 접근
- **과장 없는 ROI**: 추정치 대신 실제 고객 사례 제시
- **업종별 맞춤**: 제안서 기반 검증된 사례 DB 활용
- **Pain Point 매칭**: 회사별 특성에 맞는 사례 자동 선택

## 주요 기능

- 📊 **CSV 파일 업로드**: 회사 정보가 담긴 CSV 파일을 업로드하여 일괄 처리
  - **선택 열 지원**: `관련뉴스` (또는 `뉴스링크`, `기사링크`) - URL 입력 시 자동 스크래핑 및 메일에 활용
- 🤖 **AI 기반 조사**: Perplexity AI를 통한 실시간 회사 정보 및 최신 뉴스 수집
- 📝 **사용자 문안 모드 (NEW!)**: 사용자가 입력한 문안 + 뉴스 기반 후킹 서론 자동 생성
- ✍️ **SSR 기반 자동 생성**: Google Gemini 2.5 Pro를 활용한 4가지 스타일의 메일 문안 생성 + AI 최적안 추천
- 📰 **뉴스 후킹 (항상 적용)**: CSV 관련뉴스 또는 Perplexity 조사 결과를 활용한 구체적 뉴스 인용
- 💼 **실제 사례 포함**: 제안서 기반 검증된 PortOne 고객 사례 자동 선택 및 삽입
